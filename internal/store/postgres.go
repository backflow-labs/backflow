...
// querier abstracts over *pgxpool.Pool and pgx.Tx so all query methods
...
// PostgresStore implements the Store interface using pgxpool.
type PostgresStore struct {
	pool *pgxpool.Pool // nil when scoped to a transaction
	db   querier
}
...
// WithTx executes fn inside a database transaction. The Store passed to fn is
...
func (s *PostgresStore) WithTx(ctx context.Context, fn func(Store) error) error {
	...
	txStore := &PostgresStore{pool: nil, db: tx}
...
func (s *PostgresStore) CreateTask(ctx context.Context, task *models.Task) error {
	_, err := s.db.Exec(ctx, `
		...
		allowedToolsJSON(task.AllowedTools), task.ClaudeMD, envVarsJSON(task.EnvVars),
...
func (s *PostgresStore) ListInstances(ctx context.Context, status *models.InstanceStatus) ([]*models.Instance, error) {
	...
	for rows.Next() {
		inst, err := scanPgInstanceRows(rows)
		if err != nil {
			return nil, err
		}
...
func (s *PostgresStore) UpdateInstanceStatus(ctx context.Context, id string, status models.InstanceStatus) error {
	...
	_, err := s.db.Exec(ctx, query, status, now, id)
	return err
}
...
func (s *PostgresStore) CreateAllowedSender(ctx context.Context, sender *models.AllowedSender) error {
	_, err := s.db.Exec(ctx,
		...
		sender.ChannelType, sender.Address, sender.DefaultRepo, sender.Enabled,
		sender.CreatedAt,
	)
	return err
}
...
func scanPgTask(row pgScanner) (*models.Task, error) {
	...
	err := row.Scan(
		...
		&t.CreatePR, &t.SelfReview, &t.SaveAgentOutput, &t.PRTitle, &t.PRBody, &t.PRURL, &t.OutputURL,
	...
	json.Unmarshal(allowedToolsJSON, &t.AllowedTools)
	json.Unmarshal(envVarsJSON, &t.EnvVars)

	return &t, nil
}

func scanPgTaskRows(rows pgx.Rows) (*models.Task, error) {
	return scanPgTask(rows)
}
...
func scanPgInstanceRows(rows pgx.Rows) (*models.Instance, error) {
	return scanPgInstance(rows)
}

// --- JSON helpers ---

func allowedToolsJSON(tools []string) []byte {
	if len(tools) == 0 {
		return []byte("[]")
	}
...
func envVarsJSON(vars map[string]string) []byte {
	if len(vars) == 0 {
		return []byte("{}")
	}
