...
import (
	...
	migrationfs "github.com/backflow-labs/backflow/migrations"
...
const (
	taskColumns = `id, status, task_mode, harness, repo_url, branch, target_branch,
	...
	instanceColumns = `instance_id, instance_type, availability_zone, private_ip, status, max_containers, running_containers, created_at, updated_at`
)
...
func (s *PostgresStore) Migrate(ctx context.Context) error {
	if s.pool == nil {
		return fmt.Errorf("cannot run migrations on a transaction store")
	}
...
func (s *PostgresStore) CreateTask(ctx context.Context, task *models.Task) error {
	_, err := s.db.Exec(ctx, `
		...
		task.ReplyChannel,
		task.CreatedAt, task.UpdatedAt, nullableTimeArg(task.StartedAt), nullableTimeArg(task.CompletedAt),
	)
	return err
}
...
func (s *PostgresStore) ListTasks(ctx context.Context, filter TaskFilter) ([]*models.Task, error) {
	...
	if filter.Status != nil {
		...
		query += " WHERE status = " + placeholder(len(args))
	}
...
func (s *PostgresStore) CompleteTask(ctx context.Context, id string, result TaskResult) error {
	now := time.Now().UTC()
	_, err := s.db.Exec(ctx,
		`UPDATE tasks SET status = $1, error = $2, pr_url = $3, output_url = $4, cost_usd = $5,
		elapsed_time_sec = $6, completed_at = $7, updated_at = $8 WHERE id = $9`,
...
func (s *PostgresStore) CreateInstance(ctx context.Context, inst *models.Instance) error {
	_, err := s.db.Exec(ctx, `
		INSERT INTO instances (
			...
			status, max_containers, running_containers, created_at, updated_at
		) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
		inst.InstanceID, inst.InstanceType, inst.AvailabilityZone, inst.PrivateIP,
		inst.Status, inst.MaxContainers, inst.RunningContainers, inst.CreatedAt, inst.UpdatedAt,
	)
	return err
}

func (s *PostgresStore) GetInstance(ctx context.Context, id string) (*models.Instance, error) {
	row := s.db.QueryRow(ctx, "SELECT "+instanceColumns+" FROM instances WHERE instance_id = $1", id)
...
func (s *PostgresStore) GetAllowedSender(ctx context.Context, channelType, address string) (*models.AllowedSender, error) {
	...
	if err := row.Scan(&sender.ChannelType, &sender.Address, &sender.DefaultRepo, &sender.Enabled, &sender.CreatedAt); err != nil {
...
func scanTask(row rowScanner) (*models.Task, error) {
	var (
		task                          models.Task
		allowedToolsJSON, envVarsJSON []byte
	...
	if err := decodeJSONField(allowedToolsJSON, "[]", &task.AllowedTools); err != nil {
		return nil, fmt.Errorf("decode task allowed_tools: %w", err)
	}
	if err := decodeJSONField(envVarsJSON, "{}", &task.EnvVars); err != nil {
		return nil, fmt.Errorf("decode task env_vars: %w", err)
	}
	if startedAt.Valid {
...
func decodeJSONField(data []byte, fallback string, dest any) error {
	if len(data) == 0 {
		data = []byte(fallback)
	}
