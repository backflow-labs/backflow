...
func (m *recordingMessenger) Send(_ context.Context, msg OutboundMessage) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *failingMessenger) Send(context.Context, OutboundMessage) error {
	return m.err
}
...
func TestMessagingNotifier_SendsSMSFromEventReplyChannel(t *testing.T) {
	...
	if m.messages[0].Channel.Type != ChannelSMS {
...
func TestMessagingNotifier_SkipsWithoutReplyChannel(t *testing.T) {
	...
	mn.Notify(notify.Event{Type: notify.EventTaskCompleted, TaskID: "bf_456", Timestamp: time.Now()})
...
func TestMessagingNotifier_EventFilter(t *testing.T) {
	...
	mn.Notify(notify.Event{Type: notify.EventTaskCompleted, TaskID: "bf_789", ReplyChannel: "sms:+15551234567", Timestamp: time.Now()})
	m.mu.Lock()
	if len(m.messages) != 0 {
	...
	mn.Notify(notify.Event{Type: notify.EventTaskFailed, TaskID: "bf_789", ReplyChannel: "sms:+15551234567", Message: "something broke", Timestamp: time.Now()})
	m.mu.Lock()
	defer m.mu.Unlock()
	if len(m.messages) != 1 {
...
func TestMessagingNotifier_ErrorDoesNotPropagate(t *testing.T) {
	...
	err := mn.Notify(notify.Event{Type: notify.EventTaskCompleted, TaskID: "bf_err", ReplyChannel: "sms:+15551234567", Timestamp: time.Now()})
	if err != nil {
...
func TestMessagingNotifier_InvalidReplyChannelSkips(t *testing.T) {
	...
	mn.Notify(notify.Event{Type: notify.EventTaskCompleted, TaskID: "bf_bad", ReplyChannel: "sms", Timestamp: time.Now()})
...
func TestFormatEventMessage(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			event:    notify.Event{Type: notify.EventTaskCompleted, TaskID: "bf_1", PRURL: "https://github.com/org/repo/pull/1"},
			contains: "PR: https://github.com/org/repo/pull/1",
		},
		{
			name:     "completed without PR",
			event:    notify.Event{Type: notify.EventTaskCompleted, TaskID: "bf_1"},
			contains: "completed",
		},
		{
			name:     "failed with message",
			event:    notify.Event{Type: notify.EventTaskFailed, TaskID: "bf_1", Message: "container exited 1"},
			contains: "container exited 1",
		},
		{
			name:     "running",
			event:    notify.Event{Type: notify.EventTaskRunning, TaskID: "bf_1"},
			contains: "running",
		},
		{
			name:     "cancelled",
			event:    notify.Event{Type: notify.EventTaskCancelled, TaskID: "bf_1"},
			contains: "cancelled",
		},
	}
...
func TestFormatEventMessage_ReadCompletionIncludesTLDRAndTags(t *testing.T) {
	msg := formatEventMessage(notify.Event{
		Type:     notify.EventTaskCompleted,
		TaskMode: "read",
		TLDR:     "Short summary",
