...
| `handlers_test.go` | Tests for API handlers — health check, create/get task, list tasks, input validation, 404 on missing task, and delete. Uses an in-memory SQLite store and `httptest`. |
...
| File | Description |
|------|-------------|
| `config.go` | Defines the `Config` struct with all server settings (mode, auth, AWS, agent defaults, webhooks, DB, polling). `Load()` reads from environment variables with sensible defaults. Supports two modes (`ec2`, `local`) and two auth modes (`api_key`, `max_subscription`). `MaxConcurrent()` computes the concurrency limit based on auth mode, dispatch mode, and instance capacity. |
...
| File | Description |
|------|-------------|
| `task.go` | `Task` struct with all fields (ID, status, repo, branch, prompt, model, effort, budget, runtime, turns, PR info, container/instance IDs, cost, timestamps). `CreateTaskRequest` struct with `Validate()` for API input. `TaskStatus` enum: `pending`, `provisioning`, `running`, `completed`, `failed`, `interrupted`, `cancelled`. Helper methods `IsTerminal()`, `AllowedToolsJSON()`, `EnvVarsJSON()`. |
| `task_test.go` | Table-driven tests for `CreateTaskRequest.Validate()` (valid input, missing fields, negative budget) and `TaskStatus.IsTerminal()` for all status values. |
