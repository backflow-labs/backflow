...
- **store/** — `Store` interface + Postgres (`pgxpool`, goose migrations)
...
Postgres with goose migrations. The schema lives in `migrations/*.sql` and is applied on startup before the API and orchestrator begin serving traffic.
