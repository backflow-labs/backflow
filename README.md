...
Tasks affected by spot interruptions enter `recovering` and are automatically re-queued.

## Harnesses

Tasks can run with different agent CLIs via the `harness` field:

- **`claude_code`** (default) — Claude Code CLI. Uses `--output-format stream-json` with retry logic and structured result parsing. Requires `ANTHROPIC_API_KEY` or Max subscription credentials.
- **`codex`** — OpenAI Codex CLI. Uses `--full-auto --quiet` mode. Requires `OPENAI_API_KEY`. Defaults to `gpt-5.4` model.
...
SQLite in WAL mode. The database file location is configured via `BACKFLOW_DB_PATH` (default: `backflow.db`).

Schema is managed in `internal/store/sqlite.go` in the `migrate()` method. All DDL uses `CREATE TABLE IF NOT EXISTS`, so it's safe to run on every startup. There are no separate migration files.

**To add a new column:** add an idempotent `ALTER TABLE ... ADD COLUMN` statement to `migrate()`, then update the corresponding queries and model struct.
...
To reset the database, delete `backflow.db`. It will be recreated on next startup.
...
Creates: ECR repo, IAM role, security group, and launch template. Copy the `BACKFLOW_LAUNCH_TEMPLATE_ID` from the output into `.env`.
...
```bash
make db-status                                  # Database state
curl http://localhost:8080/api/v1/tasks/{id}    # Task details
curl http://localhost:8080/api/v1/tasks/{id}/logs?tail=100  # Container logs
curl http://localhost:8080/api/v1/health        # Health check
aws ssm start-session --target i-0abc...        # Shell into EC2 instance
```
...
- [`docs/file-reference.md`](docs/file-reference.md) — Codebase file reference
- [`docs/sizing.md`](docs/sizing.md) — Instance sizing and cost guide
