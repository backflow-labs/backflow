...
- **Completion path uses `UpsertReading` unconditionally.** The dispatch-time guard makes the `CreateReading` vs `UpsertReading` split redundant — the only remaining path is `Force=true` (overwrite by design) or `Force=false` via a race where two read dispatches passed their lookup before either wrote, for which "upsert" is the benign outcome. The unique index on `readings.url` remains as a crash-rather-than-corrupt backstop.
