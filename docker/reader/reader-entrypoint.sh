...
if [ -z "${SUPABASE_URL:-}" ] || [ -z "${SUPABASE_READER_KEY:-}" ]; then
    echo "WARNING: SUPABASE_URL and SUPABASE_READER_KEY are not both set; read-lookup.sh and read-similar.sh will fail." >&2
fi
if [ -z "${OPENAI_API_KEY:-}" ]; then
...
cd "$WORKSPACE"
HARNESS_LOG="${WORKSPACE}/reader_output.log"
